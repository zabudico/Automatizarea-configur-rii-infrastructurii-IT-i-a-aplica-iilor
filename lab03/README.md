...
Данный отчёт документирует процесс настройки сервера, включая подготовку окружения, создание файлов, разработку и запуск двух Ansible playbook'ов, а также верификацию результатов. 
Все действия выполнены на локальной машине DESKTOP-IIHR7IH (Ubuntu 18.04.6 LTS), где Ansible работает в режиме localhost (локальное подключение).
